layui.define(function (e) {
	const initPageOptions = function () {
		...
		helper.ajax(api.options, {
			...
			$.each(response.list, function (index, item) {
				helper.addOption({
					expr: 'form.form-asset-update select[name=department]',
					text: item.name,
					val: item.value,
				});
			});
	...
	const api = {
		...
		submit: '../../../hai/asset.update/single.html',
	};
	...
	const events = {
		...
		submit: function (data) {
			...
			helper.ajax(api.submit, {
				...
				if (response.result) {
					layer.msg('资产变更已提交，请等待管理员审核', function(){
						helper.closeThisTab();
					});
				} else {
	...
	const initAssetInfo = function(data){
		...
		form.val('filter-form-asset-update', data);
	...
	const bindControlEvents = function (callback) {
		...
		//提交
		form.on('submit(filter-form-asset-update)', function (e) {
			if(_data.length==0){
				layer.msg('请添加服务、端口数据后再提交')
				return false;
			}
			const params = $.extend(e.field, {
					title: '资产变更-' + e.field.address,
					submit:true,

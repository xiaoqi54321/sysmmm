layui.define(function (e) {
	...
	const api = {
		...
		action: '../../../hai/credential/asset.add/action.html',
			...
			let _result =
				`<h2 class="layui-colla-title" style="display: flex;flex-direction: row;">
					<div style="width:100%;display: flex;flex-direction: row;justify-content: flex-start;align-items: center;">
						<span>【${_index+1}】${_asset.name} - ${_asset.address}</span>
					</div>						
				</h2>
	...
	const events = {
		...
		edit:function(){
			...
			helper.openTabsPage('资产编辑-'+address, './views/asset/edit.jsp?address='+address);
		},
		//提交
		submit: function () {
			const $ = layui.$;
		...
		action: function (e) {
			if (e && e.params && e.params.comment) {
				const $ = layui.$;
				const helper = layui.helper;
				const me = $(this);

				layer.confirm('执行【' + me.text() + '】操作？', {
					btn: ['是', '否']
				}, function () {
	...
	const bindAssetItemEvent = function(){
		const $ = layui.$;
		//资产信息刷新、编辑
		$('div.div-assets .layui-btn').click(function (e) {
